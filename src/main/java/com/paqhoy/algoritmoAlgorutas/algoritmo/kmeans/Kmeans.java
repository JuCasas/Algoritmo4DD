package com.paqhoy.algoritmoAlgorutas.algoritmo.kmeans;

import com.paqhoy.algoritmoAlgorutas.model.Cluster;
...
//        log.info( "Cluster: " + clusters.get(0).firstPedido);
        for(Cluster cluster: clusters){
            ...
            if(cluster.vehiculo.getTipo_id() == 2) listaMotos.add(cluster.firstPedido);
            if(cluster.vehiculo.getTipo_id() == 1) listaAuto.add(cluster.firstPedido);
            cont++;
        }
    ...
    private void cleanPedidos(List<Cluster> clusters){
        for(Cluster cluster : clusters){
            ...
            cluster.capacidad = 0;
            cluster.pedidos = new PriorityQueue<APedido>(40,
